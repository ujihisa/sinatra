...
SinatraはRubyで下記のように最小の労力で手早くウェブアプリケーションを作成するためのDSLです。
...
gemをインストールして、動かしてみましょう。

  sudo gem install sinatra
  ruby myapp.rb

そして、 http://localhost:4567 を見てください。
...
ルートは定義順にマッチします。 リクエストに最初にマッチしたルートが呼び出されます。
...
テンプレートは<tt>./views</tt>ディレクトリ下に配置します。
他のディレクトリを使用する場合、以下のように指定します:
...
このやり方は他のテンプレート内で部分テンプレートとして表示する時によく使用されます。

=== ファイル内テンプレート

テンプレートはソースファイルの末尾で定義することもできます。
...
表示される度に使用されます。<tt>:layout => false</tt>することでレイアウトを無効にできます。
...
Development環境として実行している場合、Sinatraは特別な<tt>not_found</tt>と<tt>error</tt>ハンドラーを
組み込んでいます。

== MIMEタイプ

<tt>send_file</tt>か静的ファイルを使う時、Sinatraが知らないMIMEタイプがある場合があります。
そのときは +mime_type+ を使ってファイル拡張子ごとに登録して下さい。
...
注意: 組み込みのSinatra::TestモジュールとSinatra::TestHarnessクラスは
...
トップレベルのDSLが名前空間を汚染したり、設定を変えてしまうこと(例:./publicや./view)がありえます。
